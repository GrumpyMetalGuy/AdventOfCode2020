...
fn run() -> Result<(), Error> {
    ...
    let matcher = Regex::new(r"(\d+)-(\d+) ([a-z]):([a-z]+)").expect("Unable to create regex");

    let mut part_one_matches: i32 = 0;
    let mut part_two_matches: i32 = 0;

    for rule in input {
        for capture in matcher.captures_iter(&rule) {
            // Part 1 processing
            let lower_count = capture.get(1).unwrap().as_str().parse::<usize>()?;
            let upper_count = capture.get(2).unwrap().as_str().parse::<usize>()?;
            let target_char = capture.get(3).unwrap().as_str().chars().nth(0).unwrap();
            let password = capture.get(4).unwrap().as_str();

            let password_letter_counts = password.chars().collect::<Counter<_>>();
            let password_letter_count = *password_letter_counts.get(&target_char).unwrap_or(&0);

            if lower_count <= password_letter_count && password_letter_count <= upper_count {
                part_one_matches += 1;
            }

            // Part 2 processing
            let first_char = password.chars().nth(lower_count - 1).unwrap();
            let second_char = password.chars().nth(upper_count - 1).unwrap();

            if (first_char != second_char)
                && (first_char == target_char || second_char == target_char)
            {
                part_two_matches += 1;
            }
        }
    ...
    println!("Part 1: match count == {}", part_one_matches);
    println!("Part 2: match count == {}", part_two_matches);
